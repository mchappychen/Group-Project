...
```{r}
ggplot(data) + 
  geom_point(aes(pop,bmr),alpha=3/7,size=3) + 
  facet_wrap(~year) + 
  theme_bw() + 
...
### Which districts have the highest participation rates?
...
coord_flip() + 
  theme_bw() + 
  labs(title="Districts with the highest participation rates in 2012")
```
...
ggplot(pr2013_highest) +
  ...
  coord_flip() + 
  theme_bw() +
  labs(title="Districts with the highest participation rate in 2013")
```
...
Michael - Michael created the models for the graphs of senior population vs bmr, analyzed each model, and made the plots look pretty.
Hongyang - Wrote the Introduction and Conlcusion, added the graphs for the districts with the highest participation rates.
