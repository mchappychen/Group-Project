...
corgis <- read_csv("school_scores.csv")
...
For year 2012: Mean = 1014, Standard Deviation = 210. We wish to find the percentage of students that met the benchmark, which is over 1550.
To do this we calculate a z-score.
...
tibble(
  ...
  ggplot() +
  geom_point(aes(x,y)) + 
  geom_vline(xintercept = 1550, colour = "Red") + 
...
```
 ...
 z-score = (1550 - 1014) / 210 = 2.5524
 
 P(SAT_Score > 1550) = 1 - P(SAT_Score < 1550) = 1 - pnorm(1550,1014,210)
 
```{r}
1 - pnorm(1550,1014,210)
```
 
 We can do the same thing for 2013:
 
```{r}
1 - pnorm(1550,1018,210)
```
 
 Based on our calculations, about 0.5% of the total number of test-takers in Connecicut passed the benchmark.

Let's compare this result with our original data set.

```{r}
data %>%
  summarize(mean_bmr = mean(bmr))
```
